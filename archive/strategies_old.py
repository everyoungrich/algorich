...
class Strategy1DRebound(BaseStrategy):
    def __init__(self, broker, gs_manager=None):
    ...
    def check_3day_line_support(self, df_daily_list):
        if len(df_daily_list) < 5: return False, "FAIL: LACK_OF_DATA"

        now = datetime.now()
        current_time = int(now.strftime("%H%M"))
        if not (1510 <= current_time <= 1520): return False, f"WAITING_TIME (Current {current_time})"

        df = pd.DataFrame(df_daily_list)[::-1].reset_index(drop=True)
        df[['clpr', 'lwpr', 'oprc', 'vol']] = df[['stck_clpr', 'stck_lwpr', 'stck_oprc', 'acml_vol']].apply(pd.to_numeric)
        
        if len(df) < 5: return False, "FAIL: LACK_OF_DATA"
            
        curr = df.iloc[-1]   
        prev = df.iloc[-2]   
        
        if curr['vol'] > prev['vol'] * 0.8: return False, "FAIL: VOLUME_NOT_DECREASED"
    ...
    def run_strategy_at_specific_time(self):
        ...
        if tot_eval is None or tot_eval == 0: return
        budget_per_stock = tot_eval * 0.25
        
        holding_codes = [h["code"] for h in holdings]
        curr_hold_count = len(holdings)
        can_buy_count = max(0, 3 - curr_hold_count)
        
        if can_buy_count <= 0 or cash < budget_per_stock * 0.9: return
        
        past_lead_stocks = self.gs_manager.get_past_lead_stocks([1, 2]) if self.gs_manager else []
        if not past_lead_stocks: return
            
        for cand in past_lead_stocks:
            if can_buy_count <= 0: break
            code = cand["code"]
            name = cand["name"]
            
            if code in holding_codes: continue
            if code in self.bought_today: continue
            
            raw_data = self.broker.get_daily_prices(code, count=250)
            is_buy, reason = self.check_3day_line_support(raw_data)
            
            if is_buy:
                curr_price = float(raw_data[0]['stck_prpr']) if raw_data else 0
                if curr_price == 0: continue
                qty = math.floor(budget_per_stock / curr_price)
                if qty > 0:
                    write_log(f"[@ALGO-RICH Agent] {name}({code}) 매수 조건 충족! (사유: {reason})")
                    if self.broker.place_order(code, qty, is_buy=True):
                        trade_id = str(uuid.uuid4())[:8]
                        if self.gs_manager: self.gs_manager.log_buy(trade_id, code, name, curr_price, qty, "1D_RB")
                        can_buy_count -= 1
    ...
    def force_time_cut(self):
        ...
        for h in holdings:
            ...
            write_log(f"[종가 타임컷] 15:18 도달 전량 매도 (1박2일 마감): {h['name']}({code})")
            if self.broker.place_order(code, h["qty"], is_buy=False):
                if self.gs_manager: self.gs_manager.log_sell(code, h["current_price"], h["qty"], is_partial=False)
                if code in self.custom_sl_tracker: del self.custom_sl_tracker[code]
            time.sleep(1)
    ...
    def monitor_and_sell_morning(self):
        ...
        if tot_eval is None:
            write_log("[@System] 네트워크 불안정으로 잔고 확인 불가. 비상 정지를 유예하고 다음 사이클로 이동합니다.")
            return
        ...
        if self.daily_starting_eval > 1 and tot_eval < self.daily_starting_eval * 0.90:
            ...
            write_log(f"🚨 [진짜 비상] 자산이 시가 대비 {drop_rate:.1%} 하락! 즉시 정지합니다.")
            import sys
            sys.exit(1)
            ...
            if is_morning and cur_price >= buy_price * 1.05 and code not in self.partial_sold_tracker:
                ...
                if code in self.custom_sl_tracker:
                    ...
                    if cur_price <= sl_price:
                        sell_qty = qty
                        reason = "장중 하드컷 손절 (진입 기준 -0.2% 붕괴)"
                        sell_triggered = True
                
            if sell_triggered and sell_qty > 0:
                write_log(f"[{reason}] {name}({code}) 시장가 매도: {sell_qty}주")
                if self.broker.place_order(code, sell_qty, is_buy=False):
                    if self.gs_manager: self.gs_manager.log_sell(code, cur_price, sell_qty, is_partial=is_partial)
    ...
    def tick(self, now):
        ...
        if now.hour == 8 and now.minute >= 20 and not self.has_executed_morning_reset:
            write_log("[@System] 08:20 세션 강제 초기화(Flush) 개시")
            # token 무효화 처리는 broker 내부 로직으로 이관하거나 무시
        ...
        if now.hour == 8 and now.minute >= 30 and not self.has_executed_morning_report:
            msg = "모든 정비 완료, 장 개시 대기 중"
            write_log(f"[@Report] {msg}")
            if self.gs_manager: self.gs_manager.log_error(msg)
            ...
            if now.hour == 15 and 10 <= now.minute <= 20:
                if now.second == 15: 
                    if not self.broker.check_market_crash():
                        ...
                        write_log("[안전장치] 코스피 신저가 추락 감지! 1D-Rebound 매수 임시 중단.")
        ...
        if ((now.hour == 15 and now.minute >= 30) or now.hour > 15) and not self.has_executed_s_class_scan:
            write_log("[@Agent] 15:30 종가 기준 S-Class 주도주 확정 및 스캐닝 시작")
