    ...
    def _naver_top_by_amount(self, date_str: str, top_n: int = 40) -> list:
        ...
        for sosok in ["0", "1"]:       # 0=KOSPI, 1=KOSDAQ
            for page in [1, 2, 3]:     # 페이지당 ~50종목
                # sise_quant.nhn = 거래량 순위 (과거 날짜 조회 지원)
                # ⚠️ sise_trans.nhn(거래대금 순위)은 date= 파라미터 미지원 — 사용 불가
                ...
                if self.gs_manager:
                    ...
                    if is_logged:
                        write_log(f"[S-Class 확정] {name}({code}) "
                                  f"거래대금순위:{mkt_rank}위 "
                                  f"등락:{price_rate:.1f}% 거래량:{vol_rate:.0f}% "
                                  f"scan_amt:{scan_amt_100m}억 "
